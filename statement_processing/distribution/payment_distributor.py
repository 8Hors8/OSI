...
class PaymentDistributor:
    ...
    def _search_children_columns(self, buffer: dict, sheet: Worksheet):
        """
            ...
            Метод работает поверх уже найденных стартовых колонок месяцев и:
            1. Сортирует месяцы по их стартовым колонкам.
            2. Определяет диапазон колонок, относящихся к каждому месяцу.
            3. Сканирует строку с заголовками подколонок (начисление / оплата / пени и т.п.).
            4. Формирует словарь подколонок для каждого месяца.
            ...
            Args:
                buffer (dict): Буфер месяцев, содержащий как минимум ключ `start_col`
                    для каждого месяца.
                sheet (Worksheet): Excel-лист ведомости (openpyxl), в котором
                    производится поиск подколонок.

            Returns:
                dict: Обновлённый `buffer` с заполненным словарём `columns`
                для каждого месяца.

            Notes:
                - Если невозможно определить диапазоны колонок месяцев, метод
                  логирует ошибку и возвращает `buffer` без изменений.
                ...
                - Метод не выбрасывает исключения, чтобы не прерывать общий процесс
                  обработки ведомости.
            """
